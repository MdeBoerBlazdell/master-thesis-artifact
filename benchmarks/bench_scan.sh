...
#SBATCH --output=scan_selection_vs_slices.out
#SBATCH --mem=24G
...
for variant in slices selection; do
    case "$variant" in
        slices)
            ...
            flags="-DSLICE"
            output="$bench_dir/scan_slices.csv"
            raw="$bench_dir/sac_scan_slices.csv"
            ;;
        selection)
            binary=bin/scan_selection
            flags=""
            output="$bench_dir/scan_selections.csv"
            raw="$bench_dir/sac_scan_selections.csv"
            ;;
    esac
